...
func main() {
	...
	go func() {
		...
		err = stream.Send(ircMsg("freenode", irc.JOIN, prefix("", "", ""), []string{"#gossel_test"}, ""))
		time.Sleep(20 * time.Second)

		err = stream.Send(ircMsg("freenode", irc.PRIVMSG, prefix("", "", ""), []string{"#gossel_test"}, "My code looks so ugly, HELP ME!"))
		if err != nil {
			log.Fatal(err)
		}
